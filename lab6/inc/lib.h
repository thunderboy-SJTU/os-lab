...
int     sys_env_set_prior(envid_t envid, uint32_t prior);
int     sys_exec(void* vph, uint32_t phnum, uint32_t esp, uint32_t eip);
int     sys_transmit_packet(uint32_t addr,int len);
int     sys_receive_packet(uint32_t addr,int* len);
int     sys_get_mac(uint8_t* macaddr);
